...
|----|--------|-------------|----------|-------|
| TD-1 | Architecture | Layering is enforced by `check-layers.sh` + the folder convention, not the compiler. Splitting into SwiftPM targets would make an upward import a compile error, but forces `public` across the whole core and ripples through every harness script (check-docs map, check-layers paths, coverage glob) for marginal gain over the script. **Decision (2026-08-27): keep the script; revisit only if this grows into multiple apps or a shared library.** | low | 2026-08-26 |
...
| TD-7 | Calendar | When any series is armed, `materializeSeries` refetches a 60-day horizon each sync (throttled ≥120s). Cheap now the source is EventKit-only (local); a future non-local source would want event-driven materialization. | low | 2026-08-27 |
